...
const GENERATED_MARKER = '# Generated by PIOS domainService. Do not edit manually.';
const PLATFORM_SERVICES_MARKER = '# Managed by PIOS API startup. Do not edit manually.';
const DOMAIN_RE =
  ...
  return {
    ...
    instructions: {
      pt: [
        ...
        'Depois clique em Verificar DNS no painel da PIOS.',
      ],
    },
  };
}
    ...
    ${frontendServiceName}:
      loadBalancer:
        ...
        servers:
          ...
          - url: "http://pios_frontend:80"
`;
}

export function buildPlatformTraefikServicesConfig() {
  const frontendUrl = process.env.TRAEFIK_FRONTEND_SERVICE_URL || 'http://pios_frontend:80';
  const apiUrl = process.env.TRAEFIK_API_SERVICE_URL || 'http://pios_api:3002';
...
export async function syncPlatformTraefikServices() {
  if (!process.env.TRAEFIK_DYNAMIC_DIR) {
    return { skipped: true, reason: 'TRAEFIK_DYNAMIC_DIR_NOT_SET' };
  }

  const dynamicDir = path.resolve(TRAEFIK_DYNAMIC_DIR);
  const configPath = path.join(dynamicDir, '00-services.yml');
  const tmpPath = `${configPath}.${process.pid}.${Date.now()}.tmp`;

  await fs.mkdir(dynamicDir, { recursive: true });
  await fs.writeFile(tmpPath, buildPlatformTraefikServicesConfig(), {
    encoding: 'utf8',
    flag: 'wx',
  });
  await fs.rename(tmpPath, configPath);

  return { skipped: false, configPath };
}
  ...
  try {
    ...
    if (!existingConfig.startsWith(GENERATED_MARKER)) {
      throw new DomainProvisioningError(
        ...
        'Ja existe uma configuracao Traefik nao gerenciada pelo PIOS para este dominio.',
        409
      );
    }
  } catch (error) {
...
export async function removeDockerDomain(domainName) {
  ...
  try {
    ...
    if (!existingConfig.startsWith(GENERATED_MARKER)) {
      throw new DomainProvisioningError(
        'TRAEFIK_CONFIG_CONFLICT',
        'A configuracao Traefik deste dominio nao foi gerada pelo PIOS.',
        409
      );
    }
  ...
  try {
    ...
    return {
      ...
      message: verified
        ...
        : dnsVerified
          ? `DNS ja aponta para a PIOS, mas o SSL ainda nao esta valido (${ssl.error || 'aguardando certificado'}).`
